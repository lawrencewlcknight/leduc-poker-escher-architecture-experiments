#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Experiment 27."""
...
MODULE = "experiments.leduc_poker.average_policy_redistillation.run"
TASK_COUNT = 3
...
WORK_ROOT=/workspace/average-policy-redistillation
...
uv venv --python 3.9 --seed /tmp/average-policy-redistillation-venv
source /tmp/average-policy-redistillation-venv/bin/activate
...
WORK_ROOT="/workspace/exp27-controller-${{BATCH_TASK_RETRY_ATTEMPT:-0}}"
REPOSITORY="$WORK_ROOT/repository"
if command -v sudo >/dev/null 2>&1; then SUDO=sudo; else SUDO=; fi
$SUDO apt-get update
...
export EXP27_REMOTE_CONTROLLER=1
exec bash gcp/run_average_policy_redistillation.sh "$CONTROLLER_ACTION"
"""
...
import sys
from experiments.leduc_poker.average_policy_redistillation.config import PRODUCTION_SEEDS
...
EOF
TASK_NAME="task_$(printf '%03d' "$TASK_INDEX")_average_policy_seed_$SOURCE_SEED"
...
def build_job(args) -> dict:
    ...
    if args.kind == "train":
        max_duration, cpu, memory, machine, disk = "43200s", 8000, 30000, "n2-standard-8", 50
    elif args.kind == "smoke":
        max_duration, cpu, memory, machine, disk = "7200s", 4000, 15000, "n2-standard-4", 30
    elif args.kind == "controller":
        max_duration, cpu, memory, machine, disk = "86400s", 1000, 1500, "e2-small", 30
    else:
        max_duration, cpu, memory, machine, disk = "7200s", 2000, 7000, "n2-standard-2", 30
    return {
        "taskGroups": [{
            "taskSpec": {
        ...
        "labels": {"experiment": "average-policy-redistillation", "stage": args.kind},
    }
