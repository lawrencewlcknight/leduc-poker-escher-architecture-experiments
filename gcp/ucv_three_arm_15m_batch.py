#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Experiment 22."""
...
MODULE = "experiments.leduc_poker.ucv_three_arm_15m_simplification.run"
TASK_COUNT = 18
...
WORK_ROOT=/workspace/ucv-three-arm-15m
...
uv venv --python 3.9 --seed /tmp/ucv-three-arm-15m-venv
source /tmp/ucv-three-arm-15m-venv/bin/activate
...
WORK_ROOT="/workspace/exp22-controller-${{BATCH_TASK_RETRY_ATTEMPT:-0}}"
REPOSITORY="$WORK_ROOT/repository"
...
export EXP22_REMOTE_CONTROLLER=1

exec bash gcp/run_ucv_three_arm_15m_simplification.sh "$CONTROLLER_ACTION"
"""
...
import sys
from experiments.leduc_poker.ucv_three_arm_15m_simplification.config import PRODUCTION_SEEDS, task_schedule
...
def build_job(args) -> dict:
    ...
    if args.kind == "train":
        max_duration, cpu, memory, machine, disk = "72000s", 8000, 30000, "n2-standard-8", 100
        ...
        max_duration, cpu, memory, machine, disk = "14400s", 4000, 15000, "n2-standard-4", 100
    return {
        "taskGroups": [
            {
                "taskSpec": {
        ...
        "labels": {"experiment": "ucv-three-arm-15m", "stage": args.kind},
    }
