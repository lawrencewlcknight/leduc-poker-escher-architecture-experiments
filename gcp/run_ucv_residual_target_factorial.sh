...
BUILDER="$SCRIPT_DIR/ucv_residual_target_factorial_batch.py"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/ucv-residual-target-factorial-smoke}"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.ucv_residual_target_factorial.run \
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 25 commit SHA}"

RUN_ID="${RUN_ID:-exp25-fact-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp25-batch.XXXXXX)"
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp25_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp25_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/exp25_train_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp25_aggregate_job.json"
    ;;
  status)
  ...
  run|resume)
    ...
    echo "Remote Experiment 25 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off."
    ;;
  orchestrate)
    [[ "${EXP25_REMOTE_CONTROLLER:-}" == "1" ]] || { echo "Internal action" >&2; exit 2; }
