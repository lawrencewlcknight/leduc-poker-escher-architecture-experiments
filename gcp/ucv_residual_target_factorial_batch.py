#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Experiment 25."""
...
MODULE = "experiments.leduc_poker.ucv_residual_target_factorial.run"
TASK_COUNT = 12
...
WORK_ROOT=/workspace/ucv-residual-target-factorial
...
uv venv --python 3.9 --seed /tmp/exp25-venv
source /tmp/exp25-venv/bin/activate
...
WORK_ROOT="/workspace/exp25-controller-${{BATCH_TASK_RETRY_ATTEMPT:-0}}"
REPOSITORY="$WORK_ROOT/repository"
...
export EXP25_REMOTE_CONTROLLER=1

exec bash gcp/run_ucv_residual_target_factorial.sh "$CONTROLLER_ACTION"
"""
...
import sys
from experiments.leduc_poker.ucv_residual_target_factorial.config import PRODUCTION_SEEDS, task_schedule
...
export EXP25_REMOTE_TASK_URI="$REMOTE_TASK"
...
def build_job(args) -> dict:
    ...
    if args.kind == "train":
        # Per-attempt hard ceiling: 54 elapsed hours for a 36-active-hour worker.
    ...
    return {
        ...
        "labels": {"experiment": "ucv-residual-target", "stage": args.kind},
    }
