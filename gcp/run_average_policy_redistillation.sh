...
BUILDER="$SCRIPT_DIR/average_policy_redistillation_batch.py"
EXP25_RUN_ID="${EXP25_RUN_ID:-exp25-fact-20260906-003836}"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/average-policy-redistillation-smoke}"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.average_policy_redistillation.run smoke --output-root "$SMOKE_OUTPUT" --no-resume
fi
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 27 commit SHA}"

RUN_ID="${RUN_ID:-exp27-distill-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp27-batch.XXXXXX)"
trap 'rm -rf "$TEMP_DIR"' EXIT

build_json() {
  python3 "$BUILDER" --kind "$1" --output "$2" --run-id "$RUN_ID" --bucket-root "$BUCKET_ROOT" --service-account "$SA_EMAIL" --repo-ref "$REPO_REF" --experiment-25-run-id "$EXP25_RUN_ID" --parallelism "$PARALLELISM" --project-id "$PROJECT_ID" --region "$REGION" --controller-action "$CONTROLLER_ACTION"
}
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp27_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp27_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/exp27_train_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp27_aggregate_job.json"
    ;;
  status)
  ...
  run|resume)
    ...
    echo "Remote Experiment 27 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off."
    ;;
  orchestrate)
    [[ "${EXP27_REMOTE_CONTROLLER:-}" == "1" ]] || { echo "Internal action" >&2; exit 2; }
