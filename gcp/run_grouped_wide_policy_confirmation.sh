...
BUILDER="$SCRIPT_DIR/grouped_wide_policy_confirmation_batch.py"
EXP29_RUN_ID="${EXP29_RUN_ID:-exp29-ce-20260912-171556}"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/grouped-wide-policy-confirmation-smoke}"
  export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/exp35-matplotlib}"
  export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/exp35-cache}"
  mkdir -p "$MPLCONFIGDIR" "$XDG_CACHE_HOME"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.grouped_wide_policy_confirmation.run \
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 35 commit SHA}"

RUN_ID="${RUN_ID:-exp35-confirm-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp35-batch.XXXXXX)"
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp35_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp35_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/exp35_train_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp35_aggregate_job.json"
    ;;
  status)
  ...
  run|resume)
    ...
    echo "Remote Experiment 35 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off."
    ;;
  orchestrate)
    [[ "${EXP35_REMOTE_CONTROLLER:-}" == "1" ]] || { echo "Internal action" >&2; exit 2; }
