#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Experiment 44."""
...
MODULE = "experiments.leduc_poker.single_ucv_escher_12h.run"
TASK_COUNT = 3
...
WORK_ROOT="/workspace/single-ucv-escher-$RETRY_ATTEMPT"
REPOSITORY="$WORK_ROOT/repository"
OUTPUT_ROOT="$WORK_ROOT/output"
VENV_ROOT="/tmp/single-ucv-escher-venv-$RETRY_ATTEMPT"
...
WORK_ROOT="/workspace/exp44-controller-${{BATCH_TASK_RETRY_ATTEMPT:-0}}"
REPOSITORY="$WORK_ROOT/repository"
if command -v sudo >/dev/null 2>&1; then SUDO=sudo; else SUDO=; fi
$SUDO apt-get update
...
exec bash gcp/run_single_ucv_escher_12h.sh "$CONTROLLER_ACTION"
"""
...
import sys
from experiments.leduc_poker.single_ucv_escher_12h.config import PRODUCTION_SEEDS, task_schedule
index = int(sys.argv[1])
candidate, seed = task_schedule(PRODUCTION_SEEDS)[index]
print("EXP44_TASK_METADATA", seed, f"task_{{index:03d}}_{{candidate}}_seed_{{seed}}")
PY
)"
read -r SOURCE_SEED TASK_NAME EXTRA_METADATA <<< "$TASK_METADATA"
EXPECTED_TASK_NAME="task_$(printf '%03d' "$TASK_INDEX")_single_ucv_escher_seed_$SOURCE_SEED"
...
export EXP44_REMOTE_TASK_URI="$REMOTE_TASK"
...
""".strip()
        ...
        action = f"""
mkdir -p "$OUTPUT_ROOT/workers"
gcloud storage rsync --recursive --exclude='(^|/)(training_states|diagnostics|historical_networks)(/|$)' "$BUCKET_ROOT/$RUN_ID/workers" "$OUTPUT_ROOT/workers"
...
def build_job(args) -> dict:
    ...
    return {
        "taskGroups": [{
            ...
            "taskCount": task_count, "parallelism": parallelism, "taskCountPerNode": 1,
        }],
        "allocationPolicy": {
            "serviceAccount": {"email": args.service_account},
            "instances": [{"policy": {
                "machineType": machine, "provisioningModel": "STANDARD",
                "bootDisk": {"sizeGb": disk, "type": "pd-balanced"},
            }}],
        },
        "logsPolicy": {"destination": "CLOUD_LOGGING"},
        "labels": {"experiment": "single-ucv-escher", "stage": args.kind},
    }
