...
BUILDER="$SCRIPT_DIR/selected_ucv_36h_confirmation_batch.py"
EXP21_RUN_ID="${EXP21_RUN_ID:-exp21-36h-20260830-141641}"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/selected-ucv-36h-smoke}"
  DEEP_CFR_REPO="${DEEP_CFR_REPO:-$REPO_DIR/../../leduc_poker_deep_cfr/leduc-poker-deep-cfr-experiments}"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.selected_ucv_36h_confirmation.run \
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 24 commit SHA}"
: "${DEEP_CFR_REPO_REF:?Set DEEP_CFR_REPO_REF to the immutable Experiment 21 Deep CFR commit}"

RUN_ID="${RUN_ID:-exp24-selected-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp24-batch.XXXXXX)"
...
build_json() {
  python3 "$BUILDER" --kind "$1" --output "$2" --run-id "$RUN_ID" \
    ...
    --experiment-21-run-id "$EXP21_RUN_ID" --parallelism "$PARALLELISM" \
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp24_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp24_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/exp24_train_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp24_aggregate_job.json"
    ;;
  status)
  ...
  run|resume)
    ...
    echo "Remote Experiment 24 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off."
    ;;
  orchestrate)
    [[ "${EXP24_REMOTE_CONTROLLER:-}" == "1" ]] || { echo "Internal action" >&2; exit 2; }
