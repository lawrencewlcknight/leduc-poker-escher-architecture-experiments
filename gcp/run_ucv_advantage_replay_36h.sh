...
BUILDER="$SCRIPT_DIR/ucv_advantage_replay_36h_batch.py"
EXP21_RUN_ID="${EXP21_RUN_ID:-exp21-36h-20260830-141641}"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/ucv-advantage-replay-36h-smoke}"
  DEEP_CFR_REPO="${DEEP_CFR_REPO:-$REPO_DIR/../../leduc_poker_deep_cfr/leduc-poker-deep-cfr-experiments}"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.ucv_advantage_replay_36h.run \
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 26 commit SHA}"
: "${DEEP_CFR_REPO_REF:?Set DEEP_CFR_REPO_REF to the immutable Experiment 21 Deep CFR commit}"

RUN_ID="${RUN_ID:-exp26-replay-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp26-batch.XXXXXX)"
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp26_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp26_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/exp26_train_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp26_aggregate_job.json"
    ;;
  status)
  ...
  run|resume)
    ...
    echo "Remote Experiment 26 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off."
    ;;
  orchestrate)
    [[ "${EXP26_REMOTE_CONTROLLER:-}" == "1" ]] || { echo "Internal action" >&2; exit 2; }
