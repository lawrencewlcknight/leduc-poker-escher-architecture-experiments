...
BUILDER="$SCRIPT_DIR/average_policy_rowwise_continuation_batch.py"
EXP29_RUN_ID="${EXP29_RUN_ID:-exp29-ce-20260912-171556}"
EXP41_RUN_ID="${EXP41_RUN_ID:-exp41-fit-20260918-180815}"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/exp42-rowwise-policy-smoke}"
  export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/exp42-matplotlib}"
  export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/exp42-cache}"
  mkdir -p "$MPLCONFIGDIR" "$XDG_CACHE_HOME"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.average_policy_rowwise_continuation.run \
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 42 commit SHA}"
...
if ! git -C "$REPO_DIR" cat-file -e \
  "${REPO_REF}:gcp/average_policy_rowwise_continuation_batch.py" 2>/dev/null; then
  echo "REPO_REF predates Experiment 42: $REPO_REF" >&2; exit 2
fi
RUN_ID="${RUN_ID:-exp42-row-$(date -u '+%Y%m%d-%H%M%S')}"
...
if [[ "$ACTION" == run || "$ACTION" == resume ]]; then
  ...
  if ! gcloud storage ls \
    "$BUCKET_ROOT/$EXP41_RUN_ID/analysis/aggregate_summary.json" >/dev/null 2>&1; then
    echo "Completed Experiment 41 analysis not found under $BUCKET_ROOT/$EXP41_RUN_ID" >&2
    exit 2
  fi
fi
...
TEMP_DIR="$(mktemp -d /tmp/exp42-batch.XXXXXX)"
trap 'rm -rf "$TEMP_DIR"' EXIT
build_json() {
  python3 "$BUILDER" --kind "$1" --output "$2" \
    --run-id "$RUN_ID" --bucket-root "$BUCKET_ROOT" --service-account "$SA_EMAIL" \
    --repo-ref "$REPO_REF" --experiment-29-run-id "$EXP29_RUN_ID" \
    --experiment-41-run-id "$EXP41_RUN_ID" --parallelism "$PARALLELISM" \
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp42_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp42_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/exp42_train_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp42_aggregate_job.json" ;;
  status)
  ...
  run|resume)
    ...
    echo "Remote Experiment 42 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off." ;;
  orchestrate)
    [[ "${EXP42_REMOTE_CONTROLLER:-}" == 1 ]] || { echo "Internal action" >&2; exit 2; }
