#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Experiment 35."""
...
MODULE = "experiments.leduc_poker.grouped_wide_policy_confirmation.run"
TASK_COUNT = 5
...
WORK_ROOT="/workspace/exp35-controller-${{BATCH_TASK_RETRY_ATTEMPT:-0}}"
REPOSITORY="$WORK_ROOT/repository"
...
export EXP35_REMOTE_CONTROLLER=1

exec bash gcp/run_grouped_wide_policy_confirmation.sh "$CONTROLLER_ACTION"
"""
...
export EXP35_REMOTE_TASK_URI="$REMOTE_TASK"
...
gcloud storage rsync --recursive \
  --exclude='(^|/)(training_states|diagnostics)(/|$)' \
  "$BUCKET_ROOT/$RUN_ID/workers" "$OUTPUT_ROOT/workers"
gcloud storage rsync --recursive \
  "$BUCKET_ROOT/$EXP29_RUN_ID/analysis" \
  "$OUTPUT_ROOT/experiment_29_reference/analysis"
python -m {MODULE} aggregate --output-root "$OUTPUT_ROOT" \
  --experiment-29-root "$OUTPUT_ROOT/experiment_29_reference"
...
def build_job(args) -> dict:
    ...
    return {
        ...
        "labels": {"experiment": "grouped-wide-confirmation", "stage": args.kind},
    }
