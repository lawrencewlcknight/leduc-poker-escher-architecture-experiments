...
BUILDER="$SCRIPT_DIR/average_policy_optimization_horizon_batch.py"
EXP29_RUN_ID="${EXP29_RUN_ID:-exp29-ce-20260912-171556}"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/exp41-average-policy-horizon-smoke}"
  export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/exp41-matplotlib}"
  export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/exp41-cache}"
  mkdir -p "$MPLCONFIGDIR" "$XDG_CACHE_HOME"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.average_policy_optimization_horizon.run \
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 41 commit SHA}"
...
if ! git -C "$REPO_DIR" cat-file -e \
  "${REPO_REF}:gcp/average_policy_optimization_horizon_batch.py" 2>/dev/null; then
  echo "REPO_REF predates Experiment 41: $REPO_REF" >&2; exit 2
fi
RUN_ID="${RUN_ID:-exp41-fit-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp41-batch.XXXXXX)"
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp41_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp41_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/exp41_train_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp41_aggregate_job.json" ;;
  status)
  ...
  run|resume)
    ...
    echo "Remote Experiment 41 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off." ;;
  orchestrate)
    [[ "${EXP41_REMOTE_CONTROLLER:-}" == 1 ]] || { echo "Internal action" >&2; exit 2; }
