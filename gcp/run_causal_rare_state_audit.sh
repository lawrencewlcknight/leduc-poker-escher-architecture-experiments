...
BUILDER="$SCRIPT_DIR/causal_rare_state_audit_batch.py"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/causal-rare-state-audit-smoke}"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.causal_rare_state_audit.run smoke --output-root "$SMOKE_OUTPUT" --no-resume
fi
...
if [[ "$ACTION" == "resume" ]]; then
  ...
  TRAIN_JOB="${RUN_ID}-audit-retry-${RESUME_TAG}"
  AGGREGATE_JOB="${RUN_ID}-reaggregate-${RESUME_TAG}"
  CONTROLLER_ACTION="orchestrate-resume"
fi

TEMP_DIR="$(mktemp -d /tmp/exp30-batch.XXXXXX)"
...
build_json train "$TEMP_DIR/audit.json"
build_json aggregate "$TEMP_DIR/aggregate.json"

case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp30_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp30_smoke_job.json"
    cp "$TEMP_DIR/audit.json" "$REPO_DIR/exp30_audit_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp30_aggregate_job.json"
    ;;
  status)
