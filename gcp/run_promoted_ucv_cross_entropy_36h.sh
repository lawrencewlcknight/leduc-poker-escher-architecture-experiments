...
BUILDER="$SCRIPT_DIR/promoted_ucv_cross_entropy_36h_batch.py"
EXP21_RUN_ID="${EXP21_RUN_ID:-exp21-36h-20260830-141641}"
EXP24_RUN_ID="${EXP24_RUN_ID:-exp24-selected-20260905-222025}"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/promoted-ucv-ce-36h-smoke}"
  DEEP_CFR_REPO="${DEEP_CFR_REPO:-$REPO_DIR/../../leduc_poker_deep_cfr/leduc-poker-deep-cfr-experiments}"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.promoted_ucv_cross_entropy_36h.run \
    smoke --output-root "$SMOKE_OUTPUT" --deep-cfr-repo "$DEEP_CFR_REPO" --no-resume
fi
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 29 commit SHA}"
: "${DEEP_CFR_REPO_REF:?Set DEEP_CFR_REPO_REF to the immutable Experiment 21 Deep CFR commit}"

RUN_ID="${RUN_ID:-exp29-ce-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp29-batch.XXXXXX)"
trap 'rm -rf "$TEMP_DIR"' EXIT

build_json() {
  python3 "$BUILDER" --kind "$1" --output "$2" --run-id "$RUN_ID" \
    --bucket-root "$BUCKET_ROOT" --service-account "$SA_EMAIL" \
    --repo-ref "$REPO_REF" --deep-repo-ref "$DEEP_CFR_REPO_REF" \
    --experiment-21-run-id "$EXP21_RUN_ID" --experiment-24-run-id "$EXP24_RUN_ID" \
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp29_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp29_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/exp29_train_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp29_aggregate_job.json"
    ;;
  status)
  ...
  run|resume)
    ...
    echo "Remote Experiment 29 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off."
    ;;
  orchestrate)
    [[ "${EXP29_REMOTE_CONTROLLER:-}" == "1" ]] || { echo "Internal action" >&2; exit 2; }
