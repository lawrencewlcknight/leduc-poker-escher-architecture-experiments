#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Experiment 26."""
...
MODULE = "experiments.leduc_poker.ucv_advantage_replay_36h.run"
TASK_COUNT = 5
...
WORK_ROOT=/workspace/ucv-advantage-replay-36h
...
uv venv --python 3.9 --seed /tmp/ucv-advantage-replay-36h-venv
source /tmp/ucv-advantage-replay-36h-venv/bin/activate
...
WORK_ROOT="/workspace/exp26-controller-${{BATCH_TASK_RETRY_ATTEMPT:-0}}"
REPOSITORY="$WORK_ROOT/repository"
...
export EXP26_REMOTE_CONTROLLER=1

exec bash gcp/run_ucv_advantage_replay_36h.sh "$CONTROLLER_ACTION"
"""
...
import sys
from experiments.leduc_poker.ucv_advantage_replay_36h.config import PRODUCTION_SEEDS, task_schedule
...
""".strip()
        ...
        action = f"""
mkdir -p "$OUTPUT_ROOT/workers" "$OUTPUT_ROOT/experiment_21_reference"
...
python -m {MODULE} aggregate --output-root "$OUTPUT_ROOT" --experiment-21-root "$OUTPUT_ROOT/experiment_21_reference"
...
def build_job(args) -> dict:
    ...
    if args.kind == "train":
        max_duration, cpu, memory, machine, disk = "180000s", 8000, 30000, "n2-standard-8", 150
        ...
        max_duration, cpu, memory, machine, disk = "21600s", 4000, 15000, "n2-standard-4", 100
    return {
        "taskGroups": [
            {
                "taskSpec": {
        ...
        "labels": {"experiment": "ucv-advantage-replay-36h", "stage": args.kind},
    }
