...
BUILDER="$SCRIPT_DIR/single_ucv_escher_12h_batch.py"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/single-ucv-escher-12h-smoke}"
  export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/exp44-matplotlib}"
  export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/exp44-cache}"
  mkdir -p "$MPLCONFIGDIR" "$XDG_CACHE_HOME"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.single_ucv_escher_12h.run \
...
RUN_ID="${RUN_ID:-exp44-single-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp44-batch.XXXXXX)"
...
case "$ACTION" in
  ...
  run|resume)
    ...
    echo "Remote Experiment 44 controller submitted: $CONTROLLER_JOB"
