#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Experiment 29."""
...
MODULE = "experiments.leduc_poker.promoted_ucv_cross_entropy_36h.run"
TASK_COUNT = 5
...
EXP21_RUN_ID={_q(args.experiment_21_run_id)}
EXP24_RUN_ID={_q(args.experiment_24_run_id)}
WORK_ROOT=/workspace/promoted-ucv-ce-36h
...
uv venv --python 3.9 --seed /tmp/promoted-ucv-ce-36h-venv
source /tmp/promoted-ucv-ce-36h-venv/bin/activate
...
WORK_ROOT="/workspace/exp29-controller-${{BATCH_TASK_RETRY_ATTEMPT:-0}}"
REPOSITORY="$WORK_ROOT/repository"
...
export EXP21_RUN_ID={_q(args.experiment_21_run_id)}
export EXP24_RUN_ID={_q(args.experiment_24_run_id)}
export RUN_ID={_q(args.run_id)}
export PARALLELISM={_q(args.parallelism)}
export EXP29_REMOTE_CONTROLLER=1

exec bash gcp/run_promoted_ucv_cross_entropy_36h.sh "$CONTROLLER_ACTION"
"""
...
def _script(args) -> str:
    ...
    if args.kind == "smoke":
        action = f"""
python -m {MODULE} smoke --output-root "$OUTPUT_ROOT" --deep-cfr-repo "$DEEP_REPO" --no-resume
...
""".strip()
        ...
        action = f"""
mkdir -p "$OUTPUT_ROOT/workers" "$OUTPUT_ROOT/experiment_21_reference" "$OUTPUT_ROOT/experiment_24_reference"
gcloud storage rsync --recursive "$BUCKET_ROOT/$RUN_ID/workers" "$OUTPUT_ROOT/workers"
gcloud storage rsync --recursive "$BUCKET_ROOT/$EXP21_RUN_ID" "$OUTPUT_ROOT/experiment_21_reference"
gcloud storage rsync --recursive "$BUCKET_ROOT/$EXP24_RUN_ID" "$OUTPUT_ROOT/experiment_24_reference"
python -m {MODULE} aggregate --output-root "$OUTPUT_ROOT" \
  --experiment-21-root "$OUTPUT_ROOT/experiment_21_reference" \
  --experiment-24-root "$OUTPUT_ROOT/experiment_24_reference"
...
def build_job(args) -> dict:
    ...
    if args.kind == "train":
        ...
        max_duration, cpu, memory, machine, disk = "43200s", 8000, 30000, "n2-standard-8", 150
    return {
        "taskGroups": [
            {
                "taskSpec": {
        ...
        "labels": {"experiment": "promoted-ucv-ce-36h", "stage": args.kind},
    }
...
def main() -> None:
    ...
    parser.add_argument("--experiment-21-run-id", required=True)
    parser.add_argument("--experiment-24-run-id", required=True)
