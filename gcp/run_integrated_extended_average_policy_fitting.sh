...
BUILDER="$SCRIPT_DIR/integrated_extended_average_policy_fitting_batch.py"
EXP29_RUN_ID="${EXP29_RUN_ID:-exp29-ce-20260912-171556}"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/integrated-extended-average-policy-fitting-smoke}"
  export MPLCONFIGDIR="${MPLCONFIGDIR:-/tmp/exp43-matplotlib}"
  export XDG_CACHE_HOME="${XDG_CACHE_HOME:-/tmp/exp43-cache}"
  mkdir -p "$MPLCONFIGDIR" "$XDG_CACHE_HOME"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.integrated_extended_average_policy_fitting.run smoke --output-root "$SMOKE_OUTPUT" --no-resume
fi
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 43 commit SHA}"

RUN_ID="${RUN_ID:-exp43-fit-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp43-batch.XXXXXX)"
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp43_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp43_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/exp43_train_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp43_aggregate_job.json"
    ;;
  status)
  ...
  run|resume)
    ...
    echo "Remote Experiment 43 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off."
    ;;
  orchestrate)
    [[ "${EXP43_REMOTE_CONTROLLER:-}" == "1" ]] || { echo "Internal action" >&2; exit 2; }
