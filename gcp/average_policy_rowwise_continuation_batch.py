#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Experiment 42."""
...
MODULE = "experiments.leduc_poker.average_policy_rowwise_continuation.run"
...
WORK_ROOT="/workspace/rowwise-policy-continuation-$RETRY_ATTEMPT"
REPOSITORY="$WORK_ROOT/repository"
OUTPUT_ROOT="$WORK_ROOT/output"
SOURCE_ROOT="$WORK_ROOT/experiment_29_source"
REFERENCE_ROOT="$WORK_ROOT/experiment_41_reference"
VENV_ROOT="/tmp/rowwise-policy-continuation-venv-$RETRY_ATTEMPT"
...
EXP41_RUN_ID={_q(args.experiment_41_run_id)}
...
set -Eeuo pipefail
export DEBIAN_FRONTEND=noninteractive
WORK_ROOT="/workspace/exp42-controller-${{BATCH_TASK_RETRY_ATTEMPT:-0}}"
REPOSITORY="$WORK_ROOT/repository"
if command -v sudo >/dev/null 2>&1; then SUDO=sudo; else SUDO=; fi
$SUDO apt-get update
...
export EXP41_RUN_ID={_q(args.experiment_41_run_id)}
export RUN_ID={_q(args.run_id)}
export PARALLELISM={_q(args.parallelism)}
export EXP42_REMOTE_CONTROLLER=1
exec bash gcp/run_average_policy_rowwise_continuation.sh {_q(args.controller_action)}
"""
...
TASK_METADATA="$(python - "$TASK_INDEX" <<'PY' | sed -n 's/^EXP42_TASK //p' | tail -n 1
import sys
from experiments.leduc_poker.average_policy_rowwise_continuation.config import PRODUCTION_SEEDS
index = int(sys.argv[1])
seed = PRODUCTION_SEEDS[index]
print("EXP42_TASK", seed, f"task_{{index:03d}}_promoted_ucv_cross_entropy_seed_{{seed}}")
PY
)"
...
if [[ ! "$SOURCE_SEED" =~ ^[0-9]+$ || "$SOURCE_TASK" != "$EXPECTED_SOURCE" || -n "$EXTRA_METADATA" ]]; then
  echo "Invalid Experiment 42 task metadata: $TASK_METADATA" >&2
  exit 2
fi
TASK_NAME="task_$(printf '%03d' "$TASK_INDEX")_rowwise_policy_seed_$SOURCE_SEED"
...
def build_job(args):
    ...
    if args.kind in {"train", "smoke"}:
        ...
        duration, cpu, memory, machine, disk = "21600s", 1000, 1500, "e2-small", 30
    else:
        duration, cpu, memory, machine, disk = "3600s", 4000, 15000, "n2-standard-4", 50
    retry_count = 2 if args.kind == "controller" else 1
    return {
        "taskGroups": [{
            "taskSpec": {
        ...
        "labels": {"experiment": f"exp{EXPERIMENT_ID}", "stage": args.kind},
    }
...
def main():
    ...
    parser.add_argument(
        "--kind", choices=("controller", "smoke", "train", "aggregate"),
        required=True,
    )
    ...
    parser.add_argument("--experiment-41-run-id", required=True)
    parser.add_argument("--repo-url", default=REPO_URL)
    parser.add_argument("--parallelism", type=int, default=TASK_COUNT)
    parser.add_argument("--project-id", default="")
    parser.add_argument("--region", default="")
    parser.add_argument(
        "--controller-action", choices=("orchestrate", "orchestrate-resume"),
        default="orchestrate",
    )
