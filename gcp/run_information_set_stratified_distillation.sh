...
BUILDER="$SCRIPT_DIR/information_set_stratified_distillation_batch.py"
EXP25_RUN_ID="${EXP25_RUN_ID:-exp25-fact-20260906-003836}"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/information-set-stratified-distillation-smoke}"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.information_set_stratified_distillation.run smoke --output-root "$SMOKE_OUTPUT" --no-resume
fi
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 28 commit SHA}"

RUN_ID="${RUN_ID:-exp28-strat-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp28-batch.XXXXXX)"
...
case "$ACTION" in
  ...
  run|resume)
    ...
    echo "Remote Experiment 28 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off."
    ;;
  orchestrate)
    [[ "${EXP28_REMOTE_CONTROLLER:-}" == "1" ]] || { echo "Internal action" >&2; exit 2; }
