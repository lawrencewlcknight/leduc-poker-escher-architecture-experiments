#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Experiment 21."""
...
MODULE = "experiments.leduc_poker.deep_cfr_ucv_36h_plateau.run"
TASK_COUNT = 10
...
WORK_ROOT=/workspace/deep-cfr-ucv-36h
...
uv venv --python 3.9 --seed /tmp/deep-cfr-ucv-36h-venv
source /tmp/deep-cfr-ucv-36h-venv/bin/activate
...
WORK_ROOT="/workspace/exp21-controller-${{BATCH_TASK_RETRY_ATTEMPT:-0}}"
ARCH_REPO="$WORK_ROOT/architecture"
...
export EXP21_REMOTE_CONTROLLER=1

echo "Experiment 21 controller is starting $CONTROLLER_ACTION for $RUN_ID"
exec bash gcp/run_deep_cfr_ucv_36h_plateau.sh "$CONTROLLER_ACTION"
"""
...
import sys
from experiments.leduc_poker.deep_cfr_ucv_36h_plateau.config import PRODUCTION_SEEDS, task_schedule
index = int(sys.argv[1])
algorithm, seed = task_schedule(PRODUCTION_SEEDS)[index]
...
def build_job(args) -> dict:
    ...
    if args.kind == "train":
        max_duration = "180000s"  # 50 hours: bootstrap plus 36 active hours.
        cpu_milli = 8000
        memory_mib = 30000
        ...
        machine_type = "n2-standard-8"
        boot_disk_size = 100
    retries = 2 if args.kind == "controller" else 0
    return {
        "taskGroups": [
            {
                "taskSpec": {
        ...
        "labels": {"experiment": "deep-cfr-ucv-36h", "stage": args.kind},
    }
