#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Experiment 41."""
...
MODULE = "experiments.leduc_poker.average_policy_optimization_horizon.run"
...
WORK_ROOT="/workspace/average-policy-horizon-$RETRY_ATTEMPT"
REPOSITORY="$WORK_ROOT/repository"
OUTPUT_ROOT="$WORK_ROOT/output"
SOURCE_ROOT="$WORK_ROOT/experiment_29_source"
VENV_ROOT="/tmp/average-policy-horizon-venv-$RETRY_ATTEMPT"
...
set -Eeuo pipefail
export DEBIAN_FRONTEND=noninteractive
WORK_ROOT="/workspace/exp41-controller-${{BATCH_TASK_RETRY_ATTEMPT:-0}}"
REPOSITORY="$WORK_ROOT/repository"
if command -v sudo >/dev/null 2>&1; then SUDO=sudo; else SUDO=; fi
$SUDO apt-get update
...
export EXP41_REMOTE_CONTROLLER=1
exec bash gcp/run_average_policy_optimization_horizon.sh {_q(args.controller_action)}
"""
...
TASK_METADATA="$(python - "$TASK_INDEX" <<'PY' | sed -n 's/^EXP41_TASK //p' | tail -n 1
import sys
from experiments.leduc_poker.average_policy_optimization_horizon.config import PRODUCTION_SEEDS
index = int(sys.argv[1])
seed = PRODUCTION_SEEDS[index]
print("EXP41_TASK", seed, f"task_{{index:03d}}_promoted_ucv_cross_entropy_seed_{{seed}}")
PY
)"
...
if [[ ! "$SOURCE_SEED" =~ ^[0-9]+$ || "$SOURCE_TASK" != "$EXPECTED_SOURCE" || -n "$EXTRA_METADATA" ]]; then
  echo "Invalid Experiment 41 task metadata: $TASK_METADATA" >&2
  exit 2
fi
TASK_NAME="task_$(printf '%03d' "$TASK_INDEX")_policy_fit_seed_$SOURCE_SEED"
...
def build_job(args):
    ...
    if args.kind == "train":
        ...
        duration, cpu, memory, machine, disk = "86400s", 1000, 1500, "e2-small", 30
    else:
