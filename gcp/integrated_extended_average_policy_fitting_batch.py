#!/usr/bin/env python3
"""Build Google Cloud Batch jobs for Experiment 43."""
...
MODULE = "experiments.leduc_poker.integrated_extended_average_policy_fitting.run"
TASK_COUNT = 5
...
WORK_ROOT="/workspace/integrated-extended-fit-$RETRY_ATTEMPT"
REPOSITORY="$WORK_ROOT/repository"
OUTPUT_ROOT="$WORK_ROOT/output"
VENV_ROOT="/tmp/integrated-extended-fit-venv-$RETRY_ATTEMPT"
...
WORK_ROOT="/workspace/exp43-controller-${{BATCH_TASK_RETRY_ATTEMPT:-0}}"
REPOSITORY="$WORK_ROOT/repository"
if command -v sudo >/dev/null 2>&1; then SUDO=sudo; else SUDO=; fi
$SUDO apt-get update
...
export EXP43_REMOTE_CONTROLLER=1
exec bash gcp/run_integrated_extended_average_policy_fitting.sh "$CONTROLLER_ACTION"
"""
...
TASK_METADATA="$(python - "$TASK_INDEX" <<'PY' | sed -n 's/^EXP43_TASK_METADATA //p' | tail -n 1
import sys
from experiments.leduc_poker.integrated_extended_average_policy_fitting.config import PRODUCTION_SEEDS, task_schedule
index = int(sys.argv[1])
candidate, seed = task_schedule(PRODUCTION_SEEDS)[index]
print("EXP43_TASK_METADATA", seed, f"task_{{index:03d}}_{{candidate}}_seed_{{seed}}")
PY
)"
read -r SOURCE_SEED TASK_NAME EXTRA_METADATA <<< "$TASK_METADATA"
EXPECTED_TASK_NAME="task_$(printf '%03d' "$TASK_INDEX")_extended_fit_ucv_seed_$SOURCE_SEED"
if [[ ! "$SOURCE_SEED" =~ ^[0-9]+$ || "$TASK_NAME" != "$EXPECTED_TASK_NAME" || -n "$EXTRA_METADATA" ]]; then
  echo "Invalid Experiment 43 task metadata: $TASK_METADATA" >&2
  exit 2
fi
REMOTE_TASK="$BUCKET_ROOT/$RUN_ID/workers/$TASK_NAME"
export EXP43_REMOTE_TASK_URI="$REMOTE_TASK"
...
""".strip()
        ...
        action = f"""
mkdir -p "$OUTPUT_ROOT/workers" "$OUTPUT_ROOT/experiment_29_reference/analysis"
gcloud storage rsync --recursive --exclude='(^|/)(training_states|diagnostics)(/|$)' "$BUCKET_ROOT/$RUN_ID/workers" "$OUTPUT_ROOT/workers"
gcloud storage rsync --recursive "$BUCKET_ROOT/$EXP29_RUN_ID/analysis" "$OUTPUT_ROOT/experiment_29_reference/analysis"
python -m {MODULE} aggregate --output-root "$OUTPUT_ROOT" --experiment-29-root "$OUTPUT_ROOT/experiment_29_reference"
...
def build_job(args) -> dict:
    ...
    if args.kind == "train":
        max_duration, cpu, memory, machine, disk = "194400s", 8000, 30000, "n2-standard-8", 150
    ...
    return {
        ...
        "labels": {"experiment": "integrated-extended-fit", "stage": args.kind},
    }
