...
BUILDER="$SCRIPT_DIR/ucv_three_arm_15m_batch.py"

if [[ "$ACTION" == "smoke-local" ]]; then
  SMOKE_OUTPUT="${SMOKE_OUTPUT:-/tmp/ucv-three-arm-15m-smoke}"
  cd "$REPO_DIR"
  exec python3 -m experiments.leduc_poker.ucv_three_arm_15m_simplification.run \
...
: "${REPO_REF:?Set REPO_REF to the pushed Experiment 22 commit SHA}"

RUN_ID="${RUN_ID:-exp22-simpl-$(date -u '+%Y%m%d-%H%M%S')}"
...
TEMP_DIR="$(mktemp -d /tmp/exp22-batch.XXXXXX)"
...
case "$ACTION" in
  dry-run)
    cp "$TEMP_DIR/controller.json" "$REPO_DIR/exp22_controller_job.json"
    cp "$TEMP_DIR/smoke.json" "$REPO_DIR/exp22_smoke_job.json"
    cp "$TEMP_DIR/train.json" "$REPO_DIR/exp22_train_job.json"
    cp "$TEMP_DIR/aggregate.json" "$REPO_DIR/exp22_aggregate_job.json"
    ;;
  status)
  ...
  run|resume)
    ...
    echo "Remote Experiment 22 controller submitted: $CONTROLLER_JOB"
    echo "The laptop may now be disconnected or switched off."
    ;;
  orchestrate)
    [[ "${EXP22_REMOTE_CONTROLLER:-}" == "1" ]] || { echo "Internal action" >&2; exit 2; }
