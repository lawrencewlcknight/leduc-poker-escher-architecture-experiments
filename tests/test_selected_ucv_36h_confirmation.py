...
from experiments.leduc_poker.selected_ucv_36h_confirmation.config import (
    CANDIDATE_CONFIG,
    CANDIDATE_ID,
...
def _builder_module():
    path = REPOSITORY / "gcp" / "selected_ucv_36h_confirmation_batch.py"
    spec = importlib.util.spec_from_file_location("exp24_batch", path)
...
def test_selected_architecture_is_exactly_the_promoted_nonpredictive_core():
...
def test_batch_has_five_standard_workers_and_hard_ceiling():
    ...
    assert "selected_ucv_36h_confirmation.run worker" in script
...
def test_smoke_and_aggregate_exercise_reference_contract():
    ...
    assert "selected_ucv_36h_confirmation.run smoke" in smoke_script
    ...
    assert "selected_ucv_36h_confirmation.run aggregate" in script
...
def test_remote_controller_runs_entire_workflow():
    ...
    assert "EXP24_REMOTE_CONTROLLER=1" in script
    assert 'run_selected_ucv_36h_confirmation.sh "$CONTROLLER_ACTION"' in script
...
def test_local_run_returns_after_controller_submission(tmp_path):
    ...
    environment = {
        **os.environ,
        ...
        "RUN_ID": "exp24-test-run",
    }
    completed = subprocess.run(
        ["bash", "gcp/run_selected_ucv_36h_confirmation.sh", "run"],
        cwd=REPOSITORY,
        env=environment,
        text=True,
        capture_output=True,
    ...
    assert len(calls) == 1
    assert "batch jobs submit exp24-test-run-controller" in calls[0]
    assert "laptop may now be disconnected" in completed.stdout


def test_readme_places_experiment_24_after_experiment_23():
    readme = (REPOSITORY / "README.md").read_text()
    assert readme.index("## Experiment 23:") < readme.index("## Experiment 24:")
    section = readme.split("## Experiment 24:", 1)[1]
    assert "run_selected_ucv_36h_confirmation.sh smoke-local" in section
    assert "run_selected_ucv_36h_confirmation.sh run" in section
