"""CLI for Experiment 24 training, exact aggregation and smoke validation."""
    ...
    *, task_index: int, seeds: Sequence[int], output_root: Path, smoke: bool, resume: bool
    ...
    if resume:
        ...
        if existing is not None:
            LOGGER.info("Reusing validated Experiment 24 seed %s", seed)
            return existing
    return run_worker(seed=seed, schedule=schedule, worker_dir=worker_dir, smoke=smoke)
...
def _cmd_smoke(args) -> None:
    ...
    reference_root = output_root / "experiment_21_reference"
    subprocess.run(
        [
            sys.executable,
            "-m",
            "experiments.leduc_poker.deep_cfr_ucv_36h_plateau.run",
            "smoke",
            "--output-root",
            str(reference_root),
            "--deep-cfr-repo",
    ...
    result = aggregate_workers(
        ...
        experiment_21_root=reference_root,
        seeds=SMOKE_SEEDS,
