"""Run Experiment 10 and compare it with immutable Experiment 6 results."""
...
LOGGER = logging.getLogger("monte_carlo_control_critic_escher_5x_nodes")
RESULT_SOURCE = "experiment_10_new_run"
...
MONTE_CARLO_DIAGNOSTIC_FIELDS = (
    "mc_control_loss",
    "mc_target_count",
    "mc_target_mean",
    "mc_target_variance",
    "mc_target_abs_max",
    "mc_target_version_min",
    "mc_target_version_max",
    "mc_fold_0_seen_count",
    "mc_fold_1_seen_count",
    "mc_fold_2_seen_count",
)
DIAGNOSTIC_FIELDS = tuple(
    dict.fromkeys((*experiment_6.DIAGNOSTIC_FIELDS, *MONTE_CARLO_DIAGNOSTIC_FIELDS))
)
...
def _run_candidate(seed: int, config: Dict[str, Any], target_nodes: int):
    ...
    solver = MonteCarloControlCriticEscher(**kwargs)
    ...
    summary = {
        ...
        "final_mc_control_buffer_size": int(
    ...
    if min(solver.q_value_trainer.fold_sizes()) <= 0:
        raise RuntimeError("A Monte Carlo critic fold received no targets")
    if int(sum(solver.q_value_trainer.fold_seen_counts())) != int(
        final["mc_target_count"]
    ):
        raise RuntimeError("Monte Carlo target accounting mismatch")
    del solver
...
def _run_subprocess(run_dir, seed, config, target_nodes):
    ...
    command = [
        sys.executable,
        "-m",
        "experiments.leduc_poker.monte_carlo_control_critic_escher_5x_nodes.run",
        "--worker-input-json",
        str(input_path),
        "--worker-output-json",
        str(output_path),
...
def _plot_exploitability(run_dir, rows, *, x_key: str):
    ...
    set_chart_title(ax, f"Experiment 10 Monte Carlo critic vs Experiment 6 by {dimension}")
    ax.legend()
    fig.tight_layout()
    filename = (
...
def _plot_final(run_dir, summaries):
    ...
    set_chart_title(ax, "Experiment 10 and Experiment 6: final exploitability")
    fig.tight_layout()
    fig.savefig(run_dir / "combined_final_exploitability.png", dpi=200, bbox_inches="tight")
    plt.close(fig)


def _plot_mc_diagnostics(run_dir, rows):
    candidate = [
        row
        for row in rows
        if row["algorithm_id"] == ALGORITHM_ID
    ...
    x, target_variance, target_abs_max, q_loss = [], [], [], []
    for checkpoint in checkpoints:
        current = [
        ...
        target_variance.append(
            shared._stats(row["mc_target_variance"] for row in current)["mean"]
        )
        target_abs_max.append(
            shared._stats(row["mc_target_abs_max"] for row in current)["mean"]
        )
        q_loss.append(shared._stats(row["mc_control_loss"] for row in current)["mean"])

    fig, ax = plt.subplots(figsize=(10, 5.8))
    ax.plot(x, target_variance, marker="o", label="Return-target variance")
    ax.plot(x, q_loss, marker="o", label="Direct critic MSE loss")
    ax.set_xlabel("Nodes touched")
    ax.set_ylabel("Squared normalised utility")
    set_chart_title(ax, "Experiment 10 Monte Carlo target and fit diagnostics")
    ax.legend()
    fig.tight_layout()
    fig.savefig(run_dir / "monte_carlo_target_diagnostics.png", dpi=200, bbox_inches="tight")
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(10, 5.8))
    ax.plot(x, target_abs_max, marker="o", color="#ff7f0e")
    ax.set_xlabel("Nodes touched")
    ax.set_ylabel("Maximum absolute current-iteration target")
    set_chart_title(ax, "Experiment 10 Monte Carlo target scale")
    fig.tight_layout()
    fig.savefig(run_dir / "monte_carlo_target_scale.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def _parser():
    ...
    parser.add_argument(
        "--output-root",
        default="outputs/monte_carlo_control_critic_escher_5x_nodes",
    )
...
def main(argv: List[str] | None = None) -> int:
    ...
    run_dir = (
        ...
        / f"monte_carlo_control_critic_escher_5x_nodes_{timestamp}"
    )
    ...
    metadata = {
        ...
        "protocol": {
            "frozen_collection": "Both players collect before any regret, critic, calibration, gate, or average-policy fit changes training parameters.",
            "targets": "Every visited state/action is supervised by its recursively returned, unbiased sampled traverser value, converted to player-0 utility.",
            "cross_fitting": "A trajectory writes MC targets to one fold and uses control predictions only from the other folds.",
            "critic_fit": "Persistent critic networks use only current-iteration MC buffers and hard-sync frozen targets after all direct-regression steps.",
            "no_td": "One-step transitions emitted by the common DFS are explicitly discarded by the MC ensemble.",
            "comparison": "Immutable Experiment 6 candidate curves and summaries are checksum validated and are not retrained.",
        },
    }
    ...
    for seed in seeds:
        try:
            LOGGER.info("Running Experiment 10 seed %s to %s nodes", seed, targets[seed])
            ...
            LOGGER.error("Experiment 10 seed %s failed: %s", seed, exc)
    ...
    if combined_curves:
        ...
        _plot_mc_diagnostics(run_dir, combined_curves)
