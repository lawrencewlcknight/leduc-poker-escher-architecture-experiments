"""Configuration and immutable Experiment 6 references for Experiment 12."""
...
EXPERIMENT_ID = 12
ALGORITHM_ID = "parallel_multi_action_residual_escher"
ALGORITHM_LABEL = "Parallel Multi-Action Residual ESCHER"
...
PARALLEL_MULTI_ACTION_CONFIG = deepcopy(UNBIASED_CONFIG)
PARALLEL_MULTI_ACTION_CONFIG.update(
    {
        # The cost-regularised inclusion rule uses normalized regret-noise
        # scale / 2, clipped to [uniform_floor/|A|, 1].
        "subset_rollout_cost_scale": 2.0,
        # Leduc has at most three legal poker actions. The reference runner
        # concurrently evaluates the first multi-action frontier per traversal.
        "parallel_action_workers": 3,
    }
)
...
# Experiment 6 took 10.59 hours sequentially. Extra action branches are paid
# for by the matched node budget, while a three-worker frontier adds modest
# scheduling overhead and can reduce traversal critical-path time.
