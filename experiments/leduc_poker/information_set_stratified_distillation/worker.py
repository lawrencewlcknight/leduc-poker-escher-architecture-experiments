"""Run one source-trajectory worker for Experiment 28."""
...
from .distill import fit_information_set_policy
...
def _source_path(source_root: Path, *, seed: int, checkpoint_id: str, smoke: bool) -> Path:
    ...
    if len(matches) != 1:
        raise FileNotFoundError(
            f"Expected one source state named {filename}, found {len(matches)}"
        )
    return matches[0]
    ...
    torch.save(
        {
            ...
            "model": {
                name: value.detach().cpu() for name, value in state["model"].items()
            },
        },
        path,
    )
...
def run_worker(*, seed: int, source_root: Path, worker_dir: Path, smoke: bool) -> dict:
    ...
    metric_rows, information_rows = [], []
    decomposition_rows, sampling_rows, policies = [], [], []
    source_records = []
    ...
    for checkpoint_id in SOURCE_CHECKPOINTS:
        ...
        if source_result_paths:
            ...
            expected = [
                row
                for row in source_result.get("training_states", ())
                if row.get("filename") == path.name
            ]
            if len(expected) != 1:
                raise ValueError(
                    f"Experiment 25 source manifest does not identify {path.name}"
                )
            ...
            [
                {
                    "source_seed": seed,
                    "checkpoint_id": checkpoint_id,
                    "policy": "exact_tabular",
                    "exploitability": exact_value,
                },
                {
                    "source_seed": seed,
                    "checkpoint_id": checkpoint_id,
                    "policy": "empirical_tabular_reservoir",
                    "exploitability": empirical_value,
                },
                {
                    "source_seed": seed,
                    "checkpoint_id": checkpoint_id,
                    "policy": "archived_neural_policy",
                    "exploitability": archived_value,
                },
            ]
        )
        ...
        for replicate in fit_replicates(smoke=smoke):
            ...
            for arm_id in ARM_ORDER:
                ...
                metrics, state, details, sampler = fit_information_set_policy(
                    source,
                    game,
                    information_set_exponent=treatment["information_set_exponent"],
                    fit_seed=fit_seed,
                    ...
                    sampling_rows.append(
                        {
                            ...
                            "arm_id": arm_id,
                            "arm_label": treatment["label"],
                            "information_set_exponent": treatment[
                                "information_set_exponent"
                            ],
                            **sampler,
                        }
                    )
