"""CLI for Experiment 30 workers, aggregation and smoke validation."""
...
def _worker_dir(root: Path, index: int, seed: int) -> Path:
    return Path(root) / "workers" / f"task_{index:03d}_rare_state_audit_seed_{seed}"
    ...
    *,
    ...
    if resume and existing_path.is_file():
        ...
        if (
            ...
            and all(
                (worker_dir / relative).is_file()
                for relative in existing.get("artifacts", {}).values()
            )
        ):
            LOGGER.info("Reusing completed Experiment 30 seed %s", seed)
            return existing
    return run_worker(
        seed=seed,
...
def _cmd_smoke(args):
    ...
    LOGGER.info("Creating tiny Experiment 29-compatible source states")
