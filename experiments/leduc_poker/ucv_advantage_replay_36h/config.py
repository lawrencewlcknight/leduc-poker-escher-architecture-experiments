"""Frozen contract for Experiment 26's post-selection 36-hour follow-up."""
...
EXPERIMENT_ID = 26
EXPERIMENT_NAME = "ucv_advantage_replay_36h"
GAME_NAME = "leduc_poker"

CANDIDATE_ID = "ucv_advantage_replay"
CANDIDATE_LABEL = "UCV advantage replay"
...
CANDIDATE_CONFIG.update(
    {
        ...
        # Deep-CFR-style lifetime replay with direct, utility-normalised
        # instantaneous UCV advantage targets. Iteration weights are linear.
        "advantage_replay_weight_exponent": 1.0,
        # Keep the selected Experiment 23 fast core and make the rejected
        # optimisation package explicitly absent, isolating the replay change.
...
def validate_contract(
    *, seeds: Sequence[int], schedule: Sequence[Mapping], smoke: bool
    ...
    if tuple(dict(row) for row in schedule) != checkpoint_schedule(smoke=smoke):
        raise ValueError("Checkpoint schedule differs from the frozen Experiment 26 contract")
...
def contract_manifest() -> dict:
    return {
        ...
        "evidence_status": "architecture_development_follow_up",
        "selection_source": (
            "Experiment 23 non-predictive fast core; replay intervention motivated "
            "by Experiment 21 long-horizon volatility"
        ),
        "causal_intervention": (
            "replace recursive fitted cumulative-regret targets with persistent "
            "direct iteration-weighted instantaneous-advantage replay"
        ),
        "comparison_source": "immutable Experiment 21 trajectories",
    }
