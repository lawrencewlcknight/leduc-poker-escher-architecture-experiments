"""Exact and combined Experiment 21/26 analysis."""
...
def _validate_reference(
    ...
    if tuple(int(seed) for seed in manifest.get("seeds", ())) != tuple(int(seed) for seed in seeds):
        raise ValueError("Experiment 21 reference seeds differ from Experiment 26")
...
def _candidate_metrics(
    *, workers_root: Path, seeds: Sequence[int], smoke: bool
    ...
    for path in workers_root.rglob("worker_result.json"):
        ...
        if key in results:
            raise ValueError(f"Duplicate Experiment 26 worker result for {key}")
        if (
            result.get("status") != "complete"
            or bool(result.get("smoke")) != bool(smoke)
            or tuple(result.get("checkpoint_schedule", ())) != tuple(schedule)
        ):
            raise ValueError(f"Incomplete or mismatched Experiment 26 worker: {path}")
        results[key] = (path, result)
    if set(results) != expected:
        raise ValueError(
            f"Experiment 26 worker mismatch; missing={sorted(expected-set(results))}, "
            f"extra={sorted(set(results)-expected)}"
        )
    commits = {result["repository_commit"] for _, result in results.values()}
    if len(commits) != 1:
        raise ValueError(f"Experiment 26 workers used different commits: {sorted(commits)}")
    ...
    for key in sorted(results):
        ...
        for record in result["snapshots"]:
            ...
            if not snapshot.is_file() or sha256(snapshot) != record["sha256"]:
                raise ValueError(f"Missing or corrupt Experiment 26 snapshot: {snapshot}")
...
def aggregate_workers(
    *, workers_root: Path, experiment_21_root: Path, seeds: Sequence[int], output_dir: Path, smoke: bool
    ...
    result = {
        ...
        "inferential_note": (
            "Experiment 26 reuses the five Experiment 21 seeds for paired post-selection "
