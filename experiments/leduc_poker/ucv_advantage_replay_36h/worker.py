"""Train one UCV direct-advantage-replay Experiment 26 trajectory."""
...
from unbiased_escher.advantage_replay import (
...
DIAGNOSTIC_FIELDS = (
    ...
    "advantage_replay_seen_player_0",
    "advantage_replay_seen_player_1",
    "advantage_replay_retained_player_0",
    "advantage_replay_retained_player_1",
    "advantage_replay_capacity_player_0",
    "advantage_replay_capacity_player_1",
    "advantage_replay_weight_mean_player_0",
    "advantage_replay_weight_mean_player_1",
    "advantage_replay_weight_max_player_0",
    "advantage_replay_weight_max_player_1",
    "advantage_target_abs_mean_player_0",
    "advantage_target_abs_mean_player_1",
    "advantage_target_rms_player_0",
    "advantage_target_rms_player_1",
    "advantage_replay_weight_exponent",
    "uses_recursive_regret_target",
)


class DiagnosticAdvantageReplayUCVSolver(
    DiagnosticUCVSolver, AdvantageReplayUnbiasedControlVariateEscher
):
    """Direct-advantage-replay UCV implementation with the existing passive diagnostics."""
...
def _make_solver(seed: int, config: Mapping[str, Any]) -> DiagnosticAdvantageReplayUCVSolver:
    control_fields = {
        "max_num_iterations",
    ...
    solver = DiagnosticAdvantageReplayUCVSolver(**kwargs)
...
def _save_checkpoint(
    *,
    ...
    save_torch_policy_snapshot(
        ...
        arm="checkpointed",
        config=dict(config),
        stage_label=f"Experiment 26 {CANDIDATE_ID} {checkpoint_id}",
...
def _assert_replay_config(
    config: Mapping, curves: Sequence[Mapping], solver
) -> None:
    if config.get("fixed_control_variate_beta") != 1.0:
        raise RuntimeError("Experiment 26 must use fixed beta=1")
    if int(config.get("q_ensemble_size", -1)) != 2:
        raise RuntimeError("Experiment 26 must use two cross-fitted critics")
    if config.get("use_instantaneous_predictor") is not False:
        raise RuntimeError("Experiment 26 must disable the instantaneous predictor")
    if config.get("use_residual_calibration") is not True:
        raise RuntimeError("Experiment 26 must retain residual calibration")
    if not np.isclose(config.get("advantage_replay_weight_exponent"), 1.0):
        raise RuntimeError("Experiment 26 must use linear replay weights")
    if any(config.get(key) is not None for key in (
        ...
        raise RuntimeError("Experiment 26 must not use the rejected stability package")
    ...
    for trainer in solver.regret_trainers:
        if trainer.uses_recursive_target is not False:
            raise RuntimeError("Recursive regret targets were unexpectedly enabled")
        if trainer.replay_is_persistent is not True:
            raise RuntimeError("Advantage replay was not persistent")
        if trainer.target_normalization != "game_utility_bound":
            raise RuntimeError("Unexpected advantage-target normalisation")
    if measured and any(
        not np.isclose(row["uses_recursive_regret_target"], 0.0)
        for row in measured
    ):
        raise RuntimeError("Recorded diagnostics indicate a recursive regret target")
...
def run_worker(
    *,
    ...
    try:
        ...
        _assert_replay_config(config, curves, solver)
