"""CLI for Experiment 44 training, aggregation, scheduling, and smoke tests."""
...
from experiments.leduc_poker.promoted_ucv_cross_entropy_36h.common import read_json, sha256
...
def _validated_existing(
    ...
    if (
        ...
        return None
    for record in result.get("incumbent_snapshots", ()):
...
def _run_task(args) -> dict:
    tasks = task_schedule(args.seeds)
    if args.task_index < 0 or args.task_index >= len(tasks):
        raise ValueError(f"Task index outside [0, {len(tasks)-1}]")
    _, seed = tasks[args.task_index]
    ...
    if args.resume:
        existing = _validated_existing(
            worker_dir, seed=seed, smoke=args.smoke, schedule=schedule
        )
        if existing is not None:
            LOGGER.info("Reusing validated Experiment 44 seed %s", seed)
            return existing
    return run_worker(
...
def _cmd_worker(args) -> None:
    ...
    print(json.dumps(_run_task(args), indent=2, default=str))
...
def _cmd_aggregate(args) -> None:
    ...
    result = aggregate_workers(
        workers_root=Path(args.output_root) / "workers", seeds=args.seeds,
...
def _cmd_smoke(args) -> None:
    ...
    args.task_index = 0
    validate_contract(
        seeds=args.seeds, schedule=checkpoint_schedule(smoke=True), smoke=True
    )
    _run_task(args)
    result = aggregate_workers(
        workers_root=Path(args.output_root) / "workers", seeds=args.seeds,
        output_dir=Path(args.output_root) / "analysis", smoke=True,
    )
    print(json.dumps(result, indent=2))
