"""Run ESCHER Exp28 against two VR-DeepCFR+ algorithms at paired node budgets."""
...
LOGGER = logging.getLogger("escher_vs_vr_deep_cfr")
...
def _run_vr(
    ...
    kwargs = {
        ...
        if key not in {"max_num_iterations", "preserve_evaluation_rng"}
    }
    kwargs.update(
        num_episodes=(
...
def _run_subprocess(
    ...
    command = [
        sys.executable,
        "-m",
        "experiments.leduc_poker.escher_vs_vr_deep_cfr_matched_nodes.run",
        "--worker-input-json",
        str(input_path),
        "--worker-output-json",
        str(output_path),
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--output-root", default="outputs/escher_vs_vr_deep_cfr_matched_nodes")
...
def main(argv: List[str] | None = None) -> int:
    ...
    run_dir = Path(args.output_root) / f"escher_vs_vr_deep_cfr_matched_nodes_{timestamp}"
