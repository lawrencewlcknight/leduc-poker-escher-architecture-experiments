"""Run Experiment 11 and compare it with immutable Experiment 6 results."""
...
LOGGER = logging.getLogger("advantage_variance_sampling_escher_5x_nodes")
RESULT_SOURCE = "experiment_11_new_run"
...
ADVANTAGE_SAMPLING_DIAGNOSTIC_FIELDS = (
    "advantage_sampler_state_count",
...
DIAGNOSTIC_FIELDS = tuple(
    dict.fromkeys((*experiment_6.DIAGNOSTIC_FIELDS, *ADVANTAGE_SAMPLING_DIAGNOSTIC_FIELDS))
)
...
def _run_candidate(seed: int, config: Dict[str, Any], target_nodes: int):
    ...
    from advantage_sampling_escher import AdvantageVarianceSamplingEscher
    ...
    solver = AdvantageVarianceSamplingEscher(**kwargs)
    ...
    if float(final["advantage_sampling_probability_min"]) < minimum_floor - 1e-12:
        raise RuntimeError("Advantage sampler violated its uniform support floor")
    if not np.isfinite(
        final["predicted_advantage_variance_proxy_ratio_vs_experiment_6"]
    ):
        raise RuntimeError("Advantage sampler variance proxy was not finite")
    del solver
...
def _run_subprocess(run_dir, seed, config, target_nodes):
    ...
    command = [
        sys.executable,
        "-m",
        "experiments.leduc_poker.advantage_variance_sampling_escher_5x_nodes.run",
        "--worker-input-json",
        str(input_path),
        "--worker-output-json",
        str(output_path),
...
def _plot_exploitability(run_dir, rows, *, x_key: str):
    ...
    set_chart_title(ax, f"Experiment 11 advantage-variance sampler vs Experiment 6 by {dimension}")
    ax.legend()
    fig.tight_layout()
    filename = (
...
def _plot_final(run_dir, summaries):
    ...
    set_chart_title(ax, "Experiment 11 and Experiment 6: final exploitability")
    fig.tight_layout()
    fig.savefig(run_dir / "combined_final_exploitability.png", dpi=200, bbox_inches="tight")
    plt.close(fig)


def _plot_advantage_sampling_diagnostics(run_dir, rows):
    candidate = [
        row
        for row in rows
        if row["algorithm_id"] == ALGORITHM_ID
    ...
    x, ratio, probability_min, probability_max, entropy = [], [], [], [], []
    for checkpoint in checkpoints:
        current = [
        ...
        probability_min.append(
            shared._stats(
                row["advantage_sampling_probability_min"] for row in current
            )["mean"]
        )
        probability_max.append(
            shared._stats(
                row["advantage_sampling_probability_max"] for row in current
            )["mean"]
        )
        entropy.append(
            shared._stats(
                row["advantage_sampling_entropy_mean"] for row in current
            )["mean"]
        )

    fig, ax = plt.subplots(figsize=(10, 5.8))
    ax.plot(x, ratio, marker="o", color="#ff7f0e")
    ax.axhline(1.0, color="black", linestyle="--", linewidth=1)
    ax.set_xlabel("Nodes touched")
    ax.set_ylabel("Predicted variance proxy ratio vs Experiment 6 sampler")
    set_chart_title(ax, "Experiment 11 centred-advantage sampling objective")
    fig.tight_layout()
    fig.savefig(
        run_dir / "advantage_sampling_variance_proxy_by_nodes.png",
        dpi=200,
        bbox_inches="tight",
    )
    plt.close(fig)
    ...
    fig, ax = plt.subplots(figsize=(10, 5.8))
    ax.plot(x, probability_min, marker="o", label="Minimum probability")
    ax.plot(x, probability_max, marker="o", label="Maximum probability")
    ax.plot(x, entropy, marker="o", label="Mean entropy")
    ax.set_xlabel("Nodes touched")
    ax.set_ylabel("Sampling diagnostic")
    set_chart_title(ax, "Experiment 11 sampling distribution diagnostics")
    ax.legend()
    fig.tight_layout()
    fig.savefig(
        run_dir / "advantage_sampling_distribution_by_nodes.png",
        dpi=200,
        bbox_inches="tight",
    )
    plt.close(fig)
...
def _parser():
    ...
    parser.add_argument(
        "--output-root",
        default="outputs/advantage_variance_sampling_escher_5x_nodes",
    )
...
def main(argv: List[str] | None = None) -> int:
    ...
    if any(seed not in EXPERIMENT_2_NODE_TARGETS for seed in seeds):
        raise ValueError("Experiment 11 supports paired seeds 0, 1 and 2")
    if args.target_nodes is not None and args.target_nodes <= 0:
        raise ValueError("target-nodes must be positive")
    config = deepcopy(ADVANTAGE_SAMPLING_CONFIG)
    ...
    run_dir = (
        ...
        / f"advantage_variance_sampling_escher_5x_nodes_{timestamp}"
    )
    ...
    metadata = {
        ...
        "protocol": {
            "single_change": "Only Experiment 6's predictable traverser-action proposal is replaced.",
            "second_moment": "Calibration residual variance plus the squared mean of G-beta*Q estimates the actual control-residual second moment.",
            "centering": "Each action score is multiplied by the exact legal-action Euclidean norm of (I-1*pi^T)e_a.",
            "full_support": "The advantage-optimal proposal is mixed with Experiment 6's unchanged uniform floor.",
            "unbiasedness": "Experiment 6's exact sampled-action importance correction and policy centering are unchanged.",
    ...
    for seed in seeds:
        try:
            LOGGER.info("Running Experiment 11 seed %s to %s nodes", seed, targets[seed])
            ...
            LOGGER.error("Experiment 11 seed %s failed: %s", seed, exc)
    ...
    if combined_curves:
        ...
        _plot_advantage_sampling_diagnostics(run_dir, combined_curves)
