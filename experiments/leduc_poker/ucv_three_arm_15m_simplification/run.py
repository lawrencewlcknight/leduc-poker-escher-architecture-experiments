"""CLI for Experiment 22 workers, smoke testing, and aggregation."""
...
def _validated_existing(
    ...
    if (
        ...
        return None
    snapshot = worker_dir / result["snapshot"]["relative_path"]
    if not snapshot.is_file() or sha256(snapshot) != result["snapshot"]["sha256"]:
        return None
...
def _run_task(
    *,
    ...
    return run_worker(
        ...
        seed=seed,
        target_nodes=100 if smoke else TARGET_NODES,
...
def _cmd_worker(args) -> None:
    validate_contract(
        seeds=args.seeds,
        target_nodes=100 if args.smoke else TARGET_NODES,
        smoke=args.smoke,
    )
    result = _run_task(
