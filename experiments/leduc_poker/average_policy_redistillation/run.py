"""CLI for Experiment 27 workers, aggregation and smoke validation."""
...
from experiments.leduc_poker.four_algorithm_heldout_benchmark.common import read_json, sha256, write_csv
...
from experiments.leduc_poker.ucv_residual_target_factorial.worker import run_worker as run_experiment_25_worker
...
def _worker_dir(root: Path, index: int, seed: int) -> Path:
    return Path(root) / "workers" / f"task_{index:03d}_average_policy_seed_{seed}"


def _run_task(*, task_index: int, seeds, output_root: Path, source_root: Path, smoke: bool, resume: bool):
    ...
    if resume and existing_path.is_file():
        ...
        if (
            ...
            valid = all((worker_dir / row["relative_path"]).is_file() and sha256(worker_dir / row["relative_path"]) == row["sha256"] for row in existing.get("policies", ()))
            if valid:
                LOGGER.info("Reusing completed Experiment 27 seed %s", seed)
                return existing
    return run_worker(seed=seed, source_root=source_root, worker_dir=worker_dir, smoke=smoke)
...
def _cmd_worker(args):
    ...
    result = _run_task(task_index=args.task_index, seeds=args.seeds, output_root=args.output_root, source_root=args.source_root, smoke=args.smoke, resume=args.resume)
...
def _cmd_aggregate(args):
    if args.smoke and args.seeds == PRODUCTION_SEEDS:
        args.seeds = SMOKE_SEEDS
    validate_contract(seeds=args.seeds, smoke=args.smoke)
    result = aggregate_workers(workers_root=Path(args.output_root) / "workers", seeds=args.seeds, output_dir=Path(args.output_root) / "analysis", smoke=args.smoke)
    print(json.dumps(result, indent=2))
...
def _cmd_smoke(args):
    ...
    source_worker = source_root / "workers" / "task_002_averaged_critic_target_seed_0"
    ...
    write_csv(source_root / "analysis" / "training_state_inventory.csv", [
    ...
    _run_task(task_index=0, seeds=SMOKE_SEEDS, output_root=output_root, source_root=source_root, smoke=True, resume=args.resume)
    result = aggregate_workers(workers_root=output_root / "workers", seeds=SMOKE_SEEDS, output_dir=output_root / "analysis", smoke=True)
    print(json.dumps(result, indent=2))
...
def _cmd_schedule(args):
    if args.smoke and args.seeds == PRODUCTION_SEEDS:
        args.seeds = SMOKE_SEEDS
    validate_contract(seeds=args.seeds, smoke=args.smoke)
    print(json.dumps({"tasks": [{"task_index": index, "source_seed": seed} for index, seed in enumerate(task_schedule(args.seeds))]}, indent=2))
