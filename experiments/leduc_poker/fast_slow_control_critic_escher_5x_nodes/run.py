"""Run Experiment 9 and compare it with immutable Experiment 6 results."""
...
LOGGER = logging.getLogger("fast_slow_control_critic_escher_5x_nodes")
RESULT_SOURCE = "experiment_9_new_run"
...
FAST_SLOW_DIAGNOSTIC_FIELDS = (
    "rho_controller_loss",
    "rho_controller_target_version",
    "rho_controller_replay_size",
    "fast_slow_rho_mean",
    "fast_slow_rho_min",
    "fast_slow_rho_max",
    ...
    "fast_critic_loss",
    "slow_critic_loss",
    "fast_critic_target_version_min",
    "fast_critic_target_version_max",
    "slow_critic_target_version_min",
    "slow_critic_target_version_max",
    "slow_critic_lifetime_seen_count",
    "fast_q_fold_0_replay_size",
    "fast_q_fold_1_replay_size",
    "fast_q_fold_2_replay_size",
)
...
def _run_candidate(seed: int, config: Dict[str, Any], target_nodes: int):
    ...
    solver = FastSlowControlCriticEscher(**kwargs)
...
def _run_subprocess(run_dir, seed, config, target_nodes):
    ...
    command = [
        sys.executable,
        "-m",
        "experiments.leduc_poker.fast_slow_control_critic_escher_5x_nodes.run",
        "--worker-input-json",
        str(input_path),
        "--worker-output-json",
        str(output_path),
...
def _plot_exploitability(run_dir, rows, *, x_key: str):
    ...
    set_chart_title(ax, f"Experiment 9 fast/slow critic vs Experiment 6 by {dimension}")
    ax.legend()
    fig.tight_layout()
    filename = (
...
def _plot_final(run_dir, summaries):
    ...
    set_chart_title(ax, "Experiment 9 and Experiment 6: final exploitability")
    fig.tight_layout()
    fig.savefig(run_dir / "combined_final_exploitability.png", dpi=200, bbox_inches="tight")
    plt.close(fig)


def _plot_critic_diagnostics(run_dir, rows):
    candidate = [
        row
        for row in rows
        if row["algorithm_id"] == ALGORITHM_ID
    ...
    x, rho, rho_se, fast, slow, mixture = [], [], [], [], [], []
    for checkpoint in checkpoints:
        current = [
        ...
        fast.append(shared._stats(row["fast_critic_sampled_mse"] for row in current)["mean"])
        slow.append(shared._stats(row["slow_critic_sampled_mse"] for row in current)["mean"])
        mixture.append(shared._stats(row["mixture_critic_sampled_mse"] for row in current)["mean"])

    x_values = np.asarray(x)
    rho_values = np.asarray(rho)
    rho_errors = np.asarray(rho_se)
    fig, ax = plt.subplots(figsize=(10, 5.8))
    ax.plot(x_values, rho_values, marker="o", color="#d62728")
    ax.fill_between(
        x_values,
        rho_values - rho_errors,
        rho_values + rho_errors,
        color="#d62728",
        alpha=0.14,
    )
    ...
    set_chart_title(ax, "Experiment 9 held-out fast/slow mixture")
    fig.tight_layout()
    fig.savefig(run_dir / "rho_by_nodes.png", dpi=200, bbox_inches="tight")
    plt.close(fig)

    fig, ax = plt.subplots(figsize=(10, 5.8))
    ax.plot(x_values, fast, marker="o", label="Fast critic")
    ax.plot(x_values, slow, marker="o", label="Slow critic")
    ax.plot(x_values, mixture, marker="o", label="Controlled mixture")
    ax.set_xlabel("Nodes touched")
    ax.set_ylabel("Sampled return prediction MSE")
    set_chart_title(ax, "Experiment 9 held-out critic error")
    ax.legend()
    fig.tight_layout()
    fig.savefig(run_dir / "critic_sampled_mse_by_nodes.png", dpi=200, bbox_inches="tight")
    plt.close(fig)
...
def _parser():
    ...
    parser.add_argument(
        "--output-root",
        default="outputs/fast_slow_control_critic_escher_5x_nodes",
    )
...
def main(argv: List[str] | None = None) -> int:
    ...
    run_dir = (
        ...
        / f"fast_slow_control_critic_escher_5x_nodes_{timestamp}"
    )
    ...
    metadata = {
        ...
        "protocol": {
            "fast_critic": "Persistent network trained only from the current outer iteration's circular replay.",
            "slow_critic": "Persistent network trained from a uniform lifetime transition reservoir.",
            "cross_fitting": "Each trajectory writes to one fold and receives fast and slow predictions only from the other folds.",
            "rho_controller": "Frozen one-iteration-lagged controller fitted to out-of-fold sampled-return errors; rho is selected before observing the return.",
            "unbiasedness": "The Experiment 6 importance-corrected residual estimator remains unbiased for every predictable convex rho.",
    ...
    for seed in seeds:
        try:
            LOGGER.info("Running Experiment 9 seed %s to %s nodes", seed, targets[seed])
            ...
            LOGGER.error("Experiment 9 seed %s failed: %s", seed, exc)
    ...
    if combined_curves:
        ...
        _plot_critic_diagnostics(run_dir, combined_curves)
