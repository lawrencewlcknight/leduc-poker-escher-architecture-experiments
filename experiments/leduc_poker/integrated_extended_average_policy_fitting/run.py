"""CLI for Experiment 43 training, aggregation, and smoke validation."""
...
def _validated_existing(
    ...
    if (
        ...
        return None
    records = result.get("ordinary_snapshots", ()) + result.get("refined_snapshots", ())
...
def _run_task(*, task_index: int, seeds: Sequence[int], output_root: Path,
              smoke: bool, resume: bool) -> dict:
    ...
    if resume:
        ...
        if existing is not None:
            LOGGER.info("Reusing validated Experiment 43 seed %s", seed)
            return existing
    return run_worker(
        seed=seed, schedule=schedule, worker_dir=worker_dir,
...
def _cmd_worker(args) -> None:
    ...
    result = _run_task(
        task_index=args.task_index, seeds=args.seeds,
        output_root=args.output_root, smoke=args.smoke, resume=args.resume,
    )
...
def _write_smoke_reference(output_root: Path) -> Path:
    ...
    metrics_path = worker_result_path.parent / result["artifacts"]["refinement_metrics"]
    with open(metrics_path, newline="", encoding="utf-8") as handle:
        metrics = list(csv.DictReader(handle))
    maximum = max(int(row["refinement_update"]) for row in metrics)
    final_rows = [row for row in metrics if int(row["refinement_update"]) == maximum]
    rows = []
    offsets = {
        "deep_cfr": 0.04,
    ...
    for source in final_rows:
        if source["checkpoint_type"] != "active_time":
            continue
        baseline = float(source["exploitability"])
            ...
            rows.append(
                {
                    ...
                    "checkpoint_target_active_hours": source["checkpoint_target_active_hours"],
                    "actual_active_hours": source["actual_active_hours"],
                    "nodes_touched": source["nodes_touched"],
                    "completed_iteration": source["completed_iteration"],
    ...
    write_json(
        ...
        {
            "status": "complete", "smoke": True,
            "contract": {"experiment_id": 29, "production_seeds": [0]},
            "fixture_role": "Experiment 43 join-path smoke only",
        },
    )
...
def _cmd_smoke(args) -> None:
    ...
    reference = _write_smoke_reference(output_root)
    result = aggregate_workers(
        workers_root=output_root / "workers",
        experiment_29_root=reference,
        seeds=SMOKE_SEEDS,
