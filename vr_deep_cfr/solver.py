...
class DeepCumuAdv:
    ...
    def evaluate(
        ...
        *,
        ...
        def normalised_action_probabilities(state):
            probabilities = self.ave_policy_trainer.action_probabilities(state)
            total = float(sum(probabilities.values()))
            return {
                action: float(probability) / total
                for action, probability in probabilities.items()
            }

        tabular_policy = tabular_policy_from_callable(
            self.game, normalised_action_probabilities
        )
        exp = exploitability.exploitability(self.game, tabular_policy)
        policy_value = expected_game_score.policy_value(
            self.game.new_initial_state(), [tabular_policy] * self.num_players
        )[0]
        wall_clock = time.perf_counter() - self._solve_start_time
        self.logger.record("exp", exp)
        self.logger.record("average_policy_value", policy_value)
    ...
    def load_game(self):
        game_names = {"LeducPoker": "leduc_poker", "leduc_poker": "leduc_poker"}
        if self.game_name not in game_names:
            raise ValueError(
                f"This integrated comparison supports Leduc only, got {self.game_name!r}."
            )
        self.poker_game = True
        return pyspiel.load_game(game_names[self.game_name])
